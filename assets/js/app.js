...
const HomeView = {
  ...
  setup() {
    ...
    const dateParts = (value) => { const text=String(value||''); return { year:text.slice(0,4), month:Number(text.slice(4,6)), day:text.slice(6,8) } }
    ...
    async function loadFestivalSchedule(){festivalLoading.value=true;festivalError.value='';try{const data=await api.getThisWeekFestivals();const start=dateParts(data.week_start),end=dateParts(data.week_end);festivalReference.value=`${start.month}월 ${Number(start.day)}일 ~ ${end.month}월 ${Number(end.day)}일`;festivalItems.value=(data.items||[]).filter((item)=>item.eventstartdate&&item.title).slice(0,4)}catch(err){festivalItems.value=[];festivalError.value=err.message||'이번 주 축제 일정을 불러오지 못했습니다.'}finally{festivalLoading.value=false}}
    ...
    onMounted(()=>{load();loadFestivalSchedule();loadWeather()}); return { categories, posts, loading, error, load, festivalItems, festivalLoading, festivalError, festivalReference, monthNames, dateParts, formatAddress, loadFestivalSchedule, weather, weatherLoading, weatherError, weatherIconUrl, loadWeather }
  },
  template: `
    <section class="space-y-12"><div class="home-intro" style="display:flex;width:100%;min-height:19rem;align-items:center;justify-content:center;flex-direction:column;padding:2rem 1rem 1.5rem;text-align:center;background:transparent;border:0;box-shadow:none"><span style="display:block;margin-bottom:1.35rem;color:#91573d;font-size:.68rem;font-weight:800;letter-spacing:.22em">PUBLIC DATA · COMMUNITY</span><h1 class="whitespace-normal sm:whitespace-nowrap" style="margin:0;color:#2d261b;font-family:'Noto Serif KR',serif;font-size:clamp(1.8rem,4.5vw,3rem);font-weight:900;line-height:1.28;letter-spacing:-.05em">서울의 모든 지역정보를 한곳에서 만나요</h1><p style="margin-top:1.55rem;color:rgba(92,82,67,.72);font-size:.82rem">한국관광공사 공공데이터 기반 · 익명 커뮤니티 · AI 지역정보 챗봇</p><div style="display:flex;justify-content:center;gap:1rem;margin-top:2rem"><RouterLink to="/info" class="btn-primary" style="min-width:12rem;min-height:3rem"><i class="fa-solid fa-compass"></i> 지역정보 둘러보기</RouterLink><RouterLink to="/board" class="btn-secondary" style="min-width:10.8rem;min-height:3rem"><i class="fa-solid fa-comments"></i> 커뮤니티 가기</RouterLink></div></div>
      <div class="home-live-divider" style="display:flex;align-items:center;gap:1rem;width:100%;color:#b58269;font:italic .7rem/1 Georgia,serif"><span style="height:1px;flex:1;background:#eaddcb"></span><em style="font-style:italic;white-space:nowrap">Live Seoul Information</em><span style="height:1px;flex:1;background:#eaddcb"></span></div>
      ...
      <div class="space-y-5"><h2 class="section-title">CATEGORY</h2><div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3"><RouterLink v-for="cat in categories" :key="cat.id" :to="{ path: '/info', query: { category: cat.id } }" class="category-btn px-4 py-4 rounded-xl text-center flex flex-col gap-2"><i :class="cat.icon" class="text-accent-terracotta"></i><span class="text-sm font-semibold">{{ cat.name }}</span></RouterLink></div></div>
